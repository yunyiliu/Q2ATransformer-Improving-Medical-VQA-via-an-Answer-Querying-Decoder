...
class ViT(nn.Module):
    ...
    def forward(self, x):
        ...
        pooler = x[:, :self.num_cls].squeeze()
        # if len(pooler.shape) > 2:
        #     pooler = torch.mean(pooler, 1)

        to_return = {'pooler_output': pooler, 'last_hidden_state': x[:, self.num_cls:]}
...
if __name__ == "__main__":
    model = ViT('B_16', pretrained=True, num_cls=1)
