...
class FieldParser:
    def __init__(
        ...
        super().__init__()
        self.args = args
        self.BASE_DIR = args.base_dir_path
        ...
        self.ans2idx = json.load(open('/root/VQA_Main/Modified_MedVQA-main/data/path_VQA/path_ans2label_create.json','r'))
    ...
    def parse_all(self, features, training=False):
        ...
        to_return['target_ids'] = torch.nn.functional.one_hot(answer_idx, 3974)
        ...
        try:
            ...
            print('can not find image', os.path.join(self.BASE_DIR, features['image_name']))
            features['image'] = np.zeros((self.args.image_width, self.args.image_height, 3), dtype=np.uint8)
            to_return["image_mask"] = 1
    ...
    def parse_path(self, features, training=False):
        to_return = {'id': str(features['qid'])}
        question = self._clean_report(features.get('question', ''))
